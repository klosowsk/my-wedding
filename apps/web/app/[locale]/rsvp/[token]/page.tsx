...
import { formatEventDate, isRsvpDeadlinePassed } from "@/lib/dates";
...
export default async function RSVPPage({
  ...
  const deadline = wedding.features.rsvpDeadline;
  const deadlineClosed = isRsvpDeadlinePassed(deadline);
  const deadlineDate = deadline ? formatEventDate(deadline, locale) : undefined;
          ...
          {!guest || deadlineClosed ? (
            <div className="corner-frame bg-warm-white border border-secondary rounded-2xl p-8 md:p-12 text-center shadow-[0_4px_16px_rgba(60,53,48,0.06)]">
              ...
              <p className="text-muted text-base mb-6">
                {deadlineClosed && guest ? t("closed") : t("invalidToken")}
              </p>
              <Link
                href={`/${locale}`}
